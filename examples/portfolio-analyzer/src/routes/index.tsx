...
function PortfolioUpload() {
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      setCsvData(rows);
      console.log('File selected:', file.name, 'Rows:', rows.length);
    } catch (err) {
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-12">
          <div
            ...
            style={{
              backgroundColor:
                'color-mix(in srgb, var(--color-mix-primary) 10%, transparent)',
            }}
          >
            <TrendingUp
              className="w-8 h-8"
              style={{ color: 'var(--color-mix-primary)' }}
            />
          </div>
          ...
          <p className="text-lg text-gray-600 dark:text-gray-400">
            Upload your portfolio CSV to get AI-powered insights and
            visualizations
          </p>
        </div>
        ...
        {csvData && (
          <>
            ...
            <button
              ...
              className="w-full px-6 py-4 text-white font-semibold rounded-lg transition-colors flex items-center justify-center gap-2 shadow-lg"
              style={{
                backgroundColor: isUploading || !selectedFile ? '#6b7280' : '#6366f1',
