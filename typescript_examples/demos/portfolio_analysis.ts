...
async function main() {
  ...
  const csvPath = resolve(__dirname, "../../sample_files/robinhood_portfolio.csv");
  const csvContent = readFileSync(csvPath);
  const csvFile = new File([csvContent], "robinhood_portfolio.csv", {
    type: "text/csv",
  });
  ...
  await sendWithCallbacks(
    mix,
    session.id,
    `Look at my portfolio in the data in @${fileInfo.url} and find the top winners and losers in Q4. Show the three most relevant plots.`,
    {
      onTool: handleTool,
      onContent: (text) => {
